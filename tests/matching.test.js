...
describe('المطابقة', () => {
  ...
  it('الزحاف المأذون فيه لا يُنقص الدرجة، ويُرجَّح دونه', () => {
    // مستفعلن ← متفعلن (خبن) في الحشو. والزحاف رخصةٌ مأذون فيها،
    // فالبيت الذي جاء عليها موزونٌ تامّ الوزن لا بيتٌ فيه عيب.
    const r = engine.stages.matchPattern(P('SLSLLLSLLSLL'), 'المسحوب');
    equal(r.score, 1, 'الخبن مأذون فيه فلا يُنقص الدرجة المعروضة');
    equal(r.feet[0].variant.id, 'khabn');

    // وليس مجانًا في الترجيح: السالم يتقدّم على المزاحَف عند تساوي
    // كل شيء آخر — وهذا موضع الفرق بين الدرجتين.
    assert(r.rankScore < 1, 'درجة الترتيب تنزل بالزحاف');
    const salim = engine.stages.matchPattern(P('LLSLLLSLLSLL'), 'المسحوب');
    equal(salim.score, 1);
    equal(salim.rankScore, 1);
    assert(salim.rankScore > r.rankScore, 'السالم أعلى ترتيبًا من المزاحَف');
  });

  it('ما وقع في غير موضعه ليس مأذونًا فيه', () => {
    // العلة لا تقع إلا في العروض والضرب. ووقوعها في الحشو مخالفة
    // تُحاسَب كاملةً — أصلُها وزيادتُها — فتنزل بالدرجة المعروضة.
    const darb = engine.stages.matchPattern(P('LLSLLLSLLSLX'), 'المسحوب');
    equal(darb.score, 1, 'التسبيغ في الضرب من البحر لا عليه');
    const hashw = engine.stages.matchPattern(P('LSXLLSLLSLL'), 'المسحوب');
    assert(hashw.score < 1, 'وفي الحشو مخالفة تُحاسَب');
  });
