...
describe('الأسئلة المعلَّقة', () => {
  it('كل ما يحتاج تحقّقًا مُعلَن لا مخفيّ', () => {
    ...
    assert(q.some((x) => x.area === 'meter'), 'البحور غير المحسومة معلَنة');
    // «فاعل» كانت البند الوحيد على مستوى التفعيلات، وقد حُسمت بتأكيد
    // المستخدم أن المقصود «فاعلن». فلا يُشترط بقاء بند تفعيلة معلَّق.
  });
});
